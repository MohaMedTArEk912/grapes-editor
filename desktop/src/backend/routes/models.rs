...
pub async fn get_models(
    ...
    let models: Vec<DataModelSchema> = project
        .data_models
        .iter()
        .filter(|m| !m.archived)
        .cloned()
...
pub async fn add_model(
    ...
    let model = DataModelSchema::new(uuid::Uuid::new_v4().to_string(), &req.name);
    ...
    let relation_type = match req.relation_type.to_lowercase().as_str() {
        ...
        other => {
            return Err(ApiError::BadRequest(format!(
                "Unknown relation type: '{}'. Use: oneToOne, oneToMany, manyToOne, manyToMany",
                other
            )))
        }
    };
